#ifndef EMC_NONPARABOLIC_ANISOTROP_VALLEY_HPP
 ...
 */
template <class T>
class emcNonParabolicAnisotropValley : public emcAbstractValley<T> {
private:
...
public:
  emcNonParabolicAnisotropValley() = delete;
   ...
   */
  emcNonParabolicAnisotropValley(std::array<T, 3> inRelEffMass,
                                 T inParticleMass, SizeType inDegFactor,
                                 T inAlpha, T inBottomEnergy = 0.)
      : relEffMass(inRelEffMass), particleMass(inParticleMass),
        degFactor(inDegFactor), alpha(inAlpha), bottomEnergy(inBottomEnergy) {
  ...
  /// returns alpha (non-Parabolicity Factor)
  ...
  T getNormWaveVec(T energy) const {
    return std::sqrt(2 * effMassCond * getGamma(energy) * constants::q) /
           constants::hbar;
  }
  ...
  /// returns velocity = h * k * vogt / (effMassCond * sqrt(1 + 4 * alpha *
  /// Gamma))
...
#endif // EMC_NONPARABOLIC_ANISOTROP_VALLEY_HPP