// adapted on 22.03.2023 by Gollner Laura
// adapted P2P function calls to use P2PRCutoff Kernel (line 1510 ff.)
// (C) from scalFMM FRotationKernel main branch

#ifndef FROTATIONKERNEL_CUTOFF_HPP
#define FROTATIONKERNEL_CUTOFF_HPP
...
class FRotationKernel : public FAbstractKernels<CellClass, ContainerClass> {
  ...
  ///////////// Translation /////////////////////////////
  FSmartPointer<FReal[P + 1]>
      M2MTranslationCoef; //< This contains some precalculated values for M2M
                          // translation
  FSmartPointer<FReal[343][P + 1]>
      M2LTranslationCoef; //< This contains some precalculated values for M2L
                          // translation
  FSmartPointer<FReal[P + 1]>
      L2LTranslationCoef; //< This contains some precalculated values for L2L
                          // translation

  ///////////// Rotation    /////////////////////////////
  FComplex<FReal> rotationExpMinusImPhi[8]
                                       [SizeArray]; //< This is the vector use
                                                    // for the rotation around z
                                                    // for the M2M (multipole)
  FComplex<FReal>
      rotationExpImPhi[8][SizeArray]; //< This is the vector use for the
                                      // rotation around z for the L2L (taylor)

  FComplex<FReal>
      rotationM2LExpMinusImPhi[343][SizeArray]; //< This is the vector use for
                                                // the rotation around z for the
                                                // M2L (multipole)
  FComplex<FReal>
      rotationM2LExpImPhi[343]
                         [SizeArray]; //< This is the vector use for the
                                      // rotation around z for the M2L (taylor)
  ...
  FReal DlmkCoefOMinusTheta[8][SizeDlmkMatrix]; //< d_lmk for Multipole reverse
                                                // rotation

  FReal DlmkCoefMTheta[8][SizeDlmkMatrix];      //< d_lmk for Local rotation
  FReal DlmkCoefMMinusTheta[8][SizeDlmkMatrix]; //< d_lmk for Local reverse
                                                // rotation

  FReal DlmkCoefM2LOTheta[343][SizeDlmkMatrix]; //< d_lmk for Multipole rotation
  FReal DlmkCoefM2LMMinusTheta[343][SizeDlmkMatrix]; //< d_lmk for Local reverse
                                                     // rotation
  ...
  void precomputeTranslationCoef() {
    { // M2M & L2L
      ...
      for (int idxLevel = 0; idxLevel < treeHeight - 1; ++idxLevel) {
        // b is the parent-child distance = norm(
        // vec(widthAtLevel,widthAtLevel,widthAtLevel))
        ...
        for (int idx = 0; idx <= P; ++idx) {
          // coef m2m = (-b)^j/j!
          M2MTranslationCoef[idxLevel][idx] =
              minus_1_pow_idx * bPowIdx / factorials[idx];
          // coef l2l = b^j/j!
            ...
            for (int idxZ = -3; idxZ <= 3; ++idxZ) {
              ...
              if (idxX < -1 || 1 < idxX || idxY < -1 || 1 < idxY || idxZ < -1 ||
                ...
                for (int idx = 0; idx <= P; ++idx) {
                  ...
                  M2LTranslationCoef[idxLevel][position][idx] =
                      factorials[idx] / bPowIdx1;
                  bPowIdx1 *= b;
                }
              }
            }
   ...
   * \f]
   * The computation is simply a multiplication per a complex number \f$ e^{-i
   * \phi m} \f$ Phi should be in [0,2pi]
   *
   ...
   * At page 5 .1
   * \f[
   * O_{l,m}( \alpha + \theta, \beta ) = \sum_{k=-l}^l{ \sqrt{
   * \frac{(l-k)!(l+k)!}{(l-|m|)!(l+|m|)!} } d^l_{km}( \theta ) O_{l,k}( \alpha,
   * \beta ) } \f] Because we store only P_lm for l >= 0 and m >= 0 we use the
   * relation of symetrie as: \f$ O_{l,-m} = \bar{ O_{l,m} } (-1)^m \f$ Theta
   * should be in [0,pi]
   *
   ...
   * At page 5 .1
   * \f[
   * M_{l,m}( \alpha + \theta, \beta ) = \sum_{k=-l}^l{ \sqrt{
   * \frac{(l-|m|)!(l+|m|)!}{(l-k)!(l+k)!} } d^l_{km}( \theta ) M_{l,k}( \alpha,
   * \beta ) } \f] Because we store only P_lm for l >= 0 and m >= 0 we use the
   * relation of symetrie as: \f$ M_{l,-m} = \bar{ M_{l,m} } (-1)^m \f$ Theta
   * should be in [0,pi]
   *
   * Remark about the structure of the structure of the matrixes
   * DlmkCoef[O/M](Minus)Theta. It is composed of "P" small matrix. The matrix
   * M(l) (0 <= l <= P) has a size of (l*2+1) It means indexes are going from -l
   * to l for column and row. l = 0: ( -0 <= m <= 0 ; -0 <= k <= 0) [X] l = 1: (
   * -1 <= m <= 1 ; -1 <= k <= 1) [X X X] [X X X] [X X X] etc. The real size of
   * such matrix is : 1x1 + 3x3 + ... + (2P+1)x(2P+1)
   */
  void precomputeRotationVectors() {
    ...
    for (int l = 0; l <= P; ++l) {
      for (int m = 0; m <= l; ++m) {
        sqrtDoubleFactorials[l][m] =
            FMath::Sqrt(factorials[l - m] * factorials[l + m]);
      }
    }
    ...
    DlmkBuild(dlmkMatrix[1 + 3][0][0 + 3],
              FMath::FPiDiv2<FReal>()); // theta = Pi/2
    DlmkInverse(dlmkMatrix[-1 + 3][0][0 + 3],
                dlmkMatrix[1 + 3][0][0 + 3]); // theta = -Pi/2
    // Then other angle
    for (int x = 1; x <= 3; ++x) {
      for (int y = 0; y <= x; ++y) {
        for (int z = 1; z <= 3; ++z) {
          const FReal inclinaison =
              FSpherical<FReal>(FPoint<FReal>(FReal(x), FReal(y), FReal(z)))
                  .getInclination();
          DlmkBuild(dlmkMatrix[x + 3][y][z + 3], inclinaison);
          // For inclinaison between ]pi/2;pi[
          DlmkZNegative(dlmkMatrix[x + 3][y][(-z) + 3],
                        dlmkMatrix[x + 3][y][z + 3]);
          // For inclinaison between ]pi;3pi/2[
          DlmkInverseZNegative(dlmkMatrix[(-x) + 3][y][(-z) + 3],
                               dlmkMatrix[x + 3][y][z + 3]);
          // For inclinaison between ]3pi/2;2pi[
          DlmkInverse(dlmkMatrix[(-x) + 3][y][z + 3],
                      dlmkMatrix[x + 3][y][z + 3]);
        }
      }
    }
    ...
    for (int idxChild = 0; idxChild < 8; ++idxChild) {
      ...
      // compute the last part with l == P
      {
        ...
        for (int m = 0; m <= P; ++m, ++index_lm) {
          const FReal mphi =
              (sph.getPhiZero2Pi() + FMath::FPiDiv2<FReal>()) * FReal(m);
          // O_{l,m}( \alpha, \beta + \phi ) = e^{-i \phi m} O_{l,m}( \alpha,
          // \beta )
          rotationExpMinusImPhi[idxChild][index_lm].setRealImag(
              FMath::Cos(-mphi), FMath::Sin(-mphi));
          // M_{l,m}( \alpha, \beta + \phi ) = e^{i \phi m} M_{l,m}( \alpha,
          // \beta )
          rotationExpImPhi[idxChild][index_lm].setRealImag(FMath::Cos(mphi),
                                                           FMath::Sin(mphi));
        }
      }
      // Then for l < P it just a copy of the previous computed vector
      {
        ...
        for (int l = 0; l < P; ++l) {
          ...
          FMemUtils::copyall(rotationExpMinusImPhi[idxChild] + index_lm,
                             rotationExpMinusImPhi[idxChild] + index_P0, l + 1);
          FMemUtils::copyall(rotationExpImPhi[idxChild] + index_lm,
                             rotationExpImPhi[idxChild] + index_P0, l + 1);
      ...
      { // Then compute the inclinaison rotation
        ...
        for (int l = 0; l <= P; ++l) {
          for (int m = 0; m <= l; ++m) {
            { // for k == 0
              const FReal d_lmk_minusTheta =
                  dlmkMatrix[-dx + 3][dy][dz + 3][l][m + P][0 + P];
              const FReal d_lmk =
                  dlmkMatrix[dx + 3][dy][dz + 3][l][m + P][0 + P];
              // \sqrt{ \frac{(l-k)!(l+k)!}{(l-|m|)!(l+|m|)!} }
              const FReal Ofactor =
                  sqrtDoubleFactorials[l][0] / sqrtDoubleFactorials[l][m];
              const FReal Mfactor =
                  sqrtDoubleFactorials[l][m] / sqrtDoubleFactorials[l][0];
              ...
              DlmkCoefOMinusTheta[idxChild][index_lmk] =
                  Ofactor * d_lmk_minusTheta;
              DlmkCoefMMinusTheta[idxChild][index_lmk] =
                  Mfactor * d_lmk_minusTheta;
            ...
            for (int k = 1; k <= l; ++k) {
              const FReal d_lm_minus_k =
                  dlmkMatrix[dx + 3][dy][dz + 3][l][m + P][-k + P];
              const FReal d_lmk =
                  dlmkMatrix[dx + 3][dy][dz + 3][l][m + P][k + P];
              const FReal d_lm_minus_k_minusTheta =
                  dlmkMatrix[-dx + 3][dy][dz + 3][l][m + P][-k + P];
              const FReal d_lmk_minusTheta =
                  dlmkMatrix[-dx + 3][dy][dz + 3][l][m + P][k + P];

              const FReal Ofactor =
                  sqrtDoubleFactorials[l][k] / sqrtDoubleFactorials[l][m];
              const FReal Mfactor =
                  sqrtDoubleFactorials[l][m] / sqrtDoubleFactorials[l][k];

              // for k negatif
              DlmkCoefOTheta[idxChild][index_lmk] =
                  Ofactor * (d_lmk + minus_1_pow_k * d_lm_minus_k);
              DlmkCoefMTheta[idxChild][index_lmk] =
                  Mfactor * (d_lmk + minus_1_pow_k * d_lm_minus_k);
              DlmkCoefOMinusTheta[idxChild][index_lmk] =
                  Ofactor *
                  (d_lmk_minusTheta + minus_1_pow_k * d_lm_minus_k_minusTheta);
              DlmkCoefMMinusTheta[idxChild][index_lmk] =
                  Mfactor *
                  (d_lmk_minusTheta + minus_1_pow_k * d_lm_minus_k_minusTheta);
              ++index_lmk;
          ...
          if (idxX < -1 || 1 < idxX || idxY < -1 || 1 < idxY || idxZ < -1 ||
            ...
            // first compute the last part with l == P
            {
              ...
              for (int m = 0; m <= P; ++m, ++index_lm) {
                const FReal mphi =
                    (sph.getPhiZero2Pi() + FMath::FPiDiv2<FReal>()) * FReal(m);
                // O_{l,m}( \alpha, \beta + \phi ) = e^{-i \phi m} O_{l,m}(
                // \alpha, \beta )
                rotationM2LExpMinusImPhi[position][index_lm].setRealImag(
                    FMath::Cos(-mphi), FMath::Sin(-mphi));
                // M_{l,m}( \alpha, \beta + \phi ) = e^{i \phi m} M_{l,m}(
                // \alpha, \beta )
                rotationM2LExpImPhi[position][index_lm].setRealImag(
                    FMath::Cos(mphi), FMath::Sin(mphi));
              }
            }
            // Then for l < P copy the subpart of the previous vector
            {
              ...
              for (int l = 0; l < P; ++l) {
                FMemUtils::copyall(
                    rotationM2LExpMinusImPhi[position] + index_lm,
                    rotationM2LExpMinusImPhi[position] + index_P0, l + 1);
              ...
              for (int l = 0; l <= P; ++l) {
                for (int m = 0; m <= l; ++m) {
                  { // k == 0
                    ...
                    DlmkCoefM2LMMinusTheta[position][index_lmk] =
                        Mfactor * d_lmk_minusTheta;
                    ++index_lmk;
                  }
                  FReal minus_1_pow_k = -1.0;
                  for (int k = 1; k <= l; ++k) {
                    ...
                    DlmkCoefM2LOTheta[position][index_lmk] =
                        Ofactor * (d_lmk + minus_1_pow_k * d_lm_minus_k);
                    DlmkCoefM2LMMinusTheta[position][index_lmk] =
                        Mfactor * (d_lmk_minusTheta +
                                   minus_1_pow_k * d_lm_minus_k_minusTheta);
                    ++index_lmk;
  ...
  // DlmkInverseZNegative computes for angle \theta ]pi;3pi/2[ using
  // d_lmk(\theta-pi) DlmkInverse computes for angle \theta ]3pi/2;2pi[ using
  // d_lmk(2pi- \theta)
  ///////////////////////////////////////////////////////

  /** Compute d_mlk for \theta = 0
   * \f[
   * d^l_{m,k}( \theta ) = \delta_{m,k,} \,\, \mbox{\textrm{ $\delta$ Kronecker
   * symbol }} \f]
   */
   ...
   * \f[
   * d^l_{m,k}( \theta ) = (-1)^{l+k} \delta_{m,k},\,\, \mbox{\textrm{ $\delta$
   * Kronecker delta } } \f]
   */
   ...
   * \f]
   */
  void DlmkZNegative(FReal dlmk[P + 1][P2 + 1][P2 + 1],
                     const FReal dlmkZPositif[P + 1][P2 + 1][P2 + 1]) const {
    for (int l = 0; l <= P; ++l) {
      for (int m = -l; m <= l; ++m) {
        // if l+m is odd
        if ((l + m) & 0x1) {
   ...
   */
  void
  DlmkInverseZNegative(FReal dlmk[P + 1][P2 + 1][P2 + 1],
                       const FReal dlmkZPositif[P + 1][P2 + 1][P2 + 1]) const {
    for (int l = 0; l <= P; ++l) {
      for (int m = -l; m <= l; ++m) {
        if ((l + m) & 0x1) {
   ...
   * \f]
   */
  void DlmkInverse(FReal dlmk[P + 1][P2 + 1][P2 + 1],
                   const FReal dlmkZPositif[P + 1][P2 + 1][P2 + 1]) const {
    for (int l = 0; l <= P; ++l) {
      for (int m = -l; m <= l; ++m) {
  ...
  void DlmkBuild(FReal dlmk[P + 1][P2 + 1][P2 + 1], const FReal inTheta) const {
    ...
    { // Equ 29
      ...
      // g{l,0} = sqrt( (2l - 1) / 2l) g{l-1,0}  for l > 0
      {
        ...
        for (int l = 1; l <= P; ++l, ++fl) {
          g[index_l0] =
              FMath::Sqrt((fl * F2 - F1) / (fl * F2)) * g[index_l0 - l];
          index_l0 += l + 1;
        }
      }
      // g{l,m} = sqrt( (l - m + 1) / (l+m)) g{l,m-1}  for l > 0, 0 < m <= l
      {
        ...
        for (int l = 1; l <= P; ++l, ++fl) {
          ...
          for (int m = 1; m <= l; ++m, ++index_lm, ++fm) {
            g[index_lm] =
                FMath::Sqrt((fl - fm + F1) / (fl + fm)) * g[index_lm - 1];
          }
          ++index_lm;
        }
      }
    ...
    { // initial condition
      // Equ 28
      // d{l,m,l} = -1^(l+m) g{l,m} (1+cos(theta))^m sin(theta)^(l-m) , For l >
      // 0, 0 <= m <= l
      int index_lm = 0;
      ...
      for (int l = 0; l <= P; ++l) {
        ...
        for (int m = 0; m <= l; ++m, ++index_lm) {
          dlmk[l][P + m][P + l] = minus_1_pow_lm * g[index_lm] *
                                  cosTheta_1_pow_m * sinTheta_pow_l_minus_m;
          // update
    ...
    { // build the rest of the matrix
      ...
      for (int l = 1; l <= P; ++l, ++fl) {
        ...
        for (int k = l; k > -l; --k, --fk) {
          ...
          //            + (m+k) sin(theta) d{l,m,k} / sqrt(l(l+1) - k(k-1))
          //            (1+cos(theta))
          FReal fm = F0;
          for (int m = 0; m < l; ++m, ++fm) {
            dlmk[l][P + m][P + k - 1] =
                (FMath::Sqrt((fl * (fl + F1) - fm * (fm + F1)) /
                             (fl * (fl + F1) - fk * (fk - F1))) *
                 dlmk[l][P + m + 1][P + k]) +
                ((fm + fk) * sinTheta * dlmk[l][P + m][P + k] /
                 (FMath::Sqrt(fl * (fl + F1) - fk * (fk - F1)) *
                  (F1 + cosTheta)));
          }
          // Equ 26
          ...
          dlmk[l][P + l][P + k - 1] =
              (fl + fk) * sinTheta * dlmk[l][P + l][P + k] /
              (FMath::Sqrt(fl * (fl + F1) - fk * (fk - F1)) * (F1 + cosTheta));
        }
        // Equ 27
        // d{l,m,k} = -1^(m+k) d{l,-m,-k}  , For l > 0, -l <= m < 0, -l <= k <=
        // l
        for (int m = -l; m < 0; ++m) {
   ...
   * Fast and accurate determination of the wigner rotation matrices in the fast
   * multipole method Formula number (22) \f[ P_{0,0} = 1 P_{l,l} = (2l-1) sin(
   * \theta ) P_{l-1,l-1} ,l \ge 0 P_{l,l-1} = (2l-1) cos( \theta ) P_{l-1,l-1}
   * ,l \ge 0 P_{l,m} = \frac{(2l-1) cos( \theta ) P_{l-1,m} - (l+m-1)
   * P_{l-2,m}x}{(l-k)} ,l \ge 1, 0 \leq m \le l-1 \f]
   */
  void computeLegendre(FReal legendre[], const FReal inCosTheta,
                       const FReal inSinTheta) const {
    ...
    FReal *FRestrict legendre_l1_m =
        legendre + 1;                            // P{l-1,m} starts with P_{1,0}
    ...
    for (int l = 2; l <= P; ++l, ++fl) {
      ...
      for (int m = 0; m < l - 1; ++m) {
        // P_{l,m} = \frac{(2l-1) cos( \theta ) P_{l-1,m} - (l+m-1)
        // P_{l-2,m}x}{(l-m)}
        *(legendre_lm++) = (l2_minus_1 * inCosTheta * (*legendre_l1_m++) -
                            (lm_minus_1++) * (*legendre_l1_m1++)) /
                           (l_minus_m--);
      }
  ...
  static void RotationYWithDlmk(FComplex<FReal> vec[], const FReal *dlmkCoef) {
    ...
    int index_lm = 0;
    for (int l = 0; l <= P; ++l) {
      const FReal *const FRestrict originalVecAtL0 =
          originalVec + (index_lm * 2);
      for (int m = 0; m <= l; ++m, ++index_lm) {
   ...
   */
  static void RotationZVectorsMul(FComplex<FReal> *FRestrict dest,
                                  const FComplex<FReal> *FRestrict src,
                                  const int inSize = SizeArray) {
    const FComplex<FReal> *const FRestrict lastElement = dest + inSize;
    const FComplex<FReal> *const FRestrict intermediateLastElement =
        dest + (inSize & ~0x3);
  ...
  FPoint<FReal> getLeafCenter(const FTreeCoordinate coordinate) const {
    return FPoint<FReal>(FReal(coordinate.getX()) * widthAtLeafLevel +
                             widthAtLeafLevelDiv2 + boxCorner.getX(),
                         FReal(coordinate.getY()) * widthAtLeafLevel +
                             widthAtLeafLevelDiv2 + boxCorner.getY(),
                         FReal(coordinate.getZ()) * widthAtLeafLevel +
                             widthAtLeafLevelDiv2 + boxCorner.getZ());
  }
...
public:
  ...
  FRotationKernel(const int inTreeHeight, const FReal inBoxWidth,
                  const FPoint<FReal> &inBoxCenter)
      : boxWidth(inBoxWidth), treeHeight(inTreeHeight),
        widthAtLeafLevel(inBoxWidth / FReal(1 << (inTreeHeight - 1))),
        widthAtLeafLevelDiv2(widthAtLeafLevel / 2),
        boxCorner(inBoxCenter.getX() - (inBoxWidth / 2),
  ...
  FRotationKernel(const FRotationKernel &other)
      : boxWidth(other.boxWidth), treeHeight(other.treeHeight),
  ...
  void P2M(typename CellClass::multipole_t *const LeafMultipole,
    ...
    const FReal i_pow_m[4] = {0, FMath::FPiDiv2<FReal>(), FMath::FPi<FReal>(),
                              -FMath::FPiDiv2<FReal>()};
    ...
    for (FSize idxPart = 0; idxPart < inParticles->getNbParticles();
         ++idxPart) {
      // P2M
      const FPoint<FReal> position(positionsX[idxPart], positionsY[idxPart],
                                   positionsZ[idxPart]);
      ...
      for (int l = 0; l <= P; ++l, ++fl) {
        ...
        for (int m = 0; m <= l; ++m, ++index_l_m) {
          const FReal magnitude =
              q_aPowL * legendre[index_l_m] / factorials[l + m];
  ...
  template <class SymbolicData>
  void
  M2M(typename CellClass::multipole_t *const FRestrict ParentMultipole,
    ...
    for (int idxChild = 0; idxChild < 8; ++idxChild) {
      ...
      if (ChildMultipoles[idxChild]) {
        // Copy the source
        FMemUtils::copyall(source_w, ChildMultipoles[idxChild]->get(),
                           SizeArray);
   ...
   * \f[
   * M_{l,m}(a-b') = \sum_{j=|m|}^{\infty}{ \frac{ (j+l)! } { b^{j+l+1} }
   * O_{j,-m}(a) } , \mbox{\textrm{ j bounded by P-l } } \f] As describe in the
   * paper, when need first to rotate the SH then transfer using the formula and
   * finaly rotate back.
   */
  void
  M2L(typename CellClass::local_expansion_t *const FRestrict NodeLocalExpansion,
      const typename CellClass::multipole_t *const FRestrict
          NeighborMultipoles[],
      const int /*inSize*/, const int inLevel) {
    ...
    for (int idxNeigh = 0; idxNeigh < 343; ++idxNeigh) {
      ...
      if (NeighborMultipoles[idxNeigh]) {
        ...
        FMemUtils::copyall(source_w, NeighborMultipoles[idxNeigh]->get(),
                           SizeArray);
        ...
        for (int l = 0; l <= P; ++l) {
          ...
          for (int m = 0; m <= l; ++m, ++index_lm) {
            ...
            for (int j = m; j <= P - l; ++j, ++index_jl, index_jm += j) {
              ...
              u_lm_real +=
                  minus_1_pow_m * coef[index_jl] * source_w[index_jm].real();
              u_lm_imag -=
                  minus_1_pow_m * coef[index_jl] * source_w[index_jm].imag();
            }
  ...
  template <class SymbolicData>
  void
  M2L(typename CellClass::local_expansion_t *const FRestrict TargetExpansion,
      ...
      const int neighborPositions[], const int inSize) {
    int level = static_cast<int>(TargetSymb->getLevel());
    // To copy the multipole data allocated once
    FComplex<FReal> source_w[SizeArray];
    // For all children
    for (int idxExistingNeigh = 0; idxExistingNeigh < inSize;
         ++idxExistingNeigh) {
      const int idxNeigh = neighborPositions[idxExistingNeigh];
      // if interaction exits
      const FReal *const coef = M2LTranslationCoef[level][idxNeigh];
      // Copy multipole data into buffer
      FMemUtils::copyall(source_w, SourceMultipoles[idxExistingNeigh]->get(),
                         SizeArray);
   ...
   * \f[
   * M_{l,m}(a-b') = \sum_{j=l}^{\infty}{ \frac{ b^{j-l} }{ (j-l)! } M_{j,m}(a)
   * } , \textrm{j bounded by P} \f] As describe in the paper, when need first
   * to rotate the SH then transfer using the formula and finaly rotate back.
   */
  template <class SymbolicData>
  void L2L(const typename CellClass::local_expansion_t *const FRestrict
               ParentLocalExpansion,
           const SymbolicData *const ParentSymb,
    ...
    for (int idxChild = 0; idxChild < 8; ++idxChild) {
      ...
      if (ChildLocalExpansion[idxChild]) {
        ...
        // Sum in child
        FMemUtils::addall(ChildLocalExpansion[idxChild]->get(), target_u,
                          SizeArray);
      }
    }
  }
   ...
   * \f[
   *  \Phi = \sum_{j=0}^P{\left( u_{j,0} I_{j,0}(r, \theta, \phi) +
   * \sum_{k=1}^j{2 Re(u_{j,k} I_{j,k}(r, \theta, \phi))} \right)},
   *  \textrm{since } u_{l,-m} = (-1)^m \overline{ u_{l,m} }
   * \f]
   *
   * The forces are coming form the formulas, page 37, formulas 2.14 + 3:
   * \f[
   * F_r = -\frac{1}{r} \left( \sum_{j=1}^P{j u_{j,0} I_{j,0}(r, \theta, \phi) }
   * + \sum_{k=1}^j{2 j Re(u_{j,k} I_{j,k}(r, \theta, \phi))} \right) F_{ \theta
   * } = -\frac{1}{r} \left( \sum_{j=0}^P{j u_{j,0} \frac{ \partial I_{j,0}(r,
   * \theta, \phi) }{ \partial \theta } } + \sum_{k=1}^j{2 Re(u_{j,k} \frac{
   * \partial I_{j,k}(r, \theta, \phi) }{ \partial \theta })} \right) F_{ \phi }
   * = -\frac{1}{r sin \phi} \sum_{j=0}^P \sum_{k=1}^j{(-2k) Im(u_{j,k}
   * I_{j,k}(r, \theta, \phi)) } \f]
   */
  template <class SymbolicData>
  void
  L2P(const typename CellClass::local_expansion_t *const LeafLocalExpansion,
      const SymbolicData *const LeafSymb, ContainerClass *const inParticles) {
    const FReal i_pow_m[4] = {0, FMath::FPiDiv2<FReal>(), FMath::FPi<FReal>(),
                              -FMath::FPiDiv2<FReal>()};
    ...
    for (FSize idxPart = 0; idxPart < inParticles->getNbParticles();
      ...
      FReal minus_r_pow_l_legendre_div_fact_lm[SizeArray];
      {
        ...
        for (int l = 0; l <= P; ++l) {
          for (int m = 0; m <= l; ++m, ++index_lm) {
            minus_r_pow_l_div_fact_lm[index_lm] =
                minus_r_pow_l / factorials[l + m];
            minus_r_pow_l_legendre_div_fact_lm[index_lm] =
                minus_r_pow_l_div_fact_lm[index_lm] * legendre[index_lm];
          }
          minus_r_pow_l *= -r;
        }
      }
      ...
      {
        for (int m = 0; m <= P; ++m) {
          const FReal m_phi_i_pow_m =
              FReal(m) * sph.getPhi() + i_pow_m[m & 0x3];
      ...
      // compute the forces
      {
        ...
        for (int l = 1; l <= P; ++l, ++fl) {
          // first m == 0
          {
            Fr += fl * u[index_lm].real() *
                  minus_r_pow_l_legendre_div_fact_lm[index_lm];
          }
          {
            const FReal coef = minus_r_pow_l_div_fact_lm[index_lm] *
                               (fl *
                                (sph.getCosTheta() * legendre[index_lm] -
                                 legendre[index_lm - l]) /
                                sph.getSinTheta());
          ...
          for (int m = 1; m <= l; ++m, ++index_lm) {
            {
              ...
              // F(r) += 2 x l x Real(LI)
              Fr += 2 * fl *
                    (u[index_lm].real() * I_real - u[index_lm].imag() * I_imag);
              // F(p) += -2 x m x Imag(LI)
              Fp -= 2 * FReal(m) *
                    (u[index_lm].real() * I_imag + u[index_lm].imag() * I_real);
            }
            {
              const FReal legendre_l_minus_1 =
                  (m == l) ? FReal(0.0) : FReal(l + m) * legendre[index_lm - l];
              const FReal coef = minus_r_pow_l_div_fact_lm[index_lm] *
                                 ((fl * sph.getCosTheta() * legendre[index_lm] -
                                   legendre_l_minus_1) /
                                  sph.getSinTheta());
              const FReal dI_real = coef * cos_m_phi_i_pow_m[m];
              const FReal dI_imag = coef * sin_m_phi_i_pow_m[m];
              // F(O) += 2 * Real(L dI/dO)
              FO += FReal(2.0) * (u[index_lm].real() * dI_real -
                                  u[index_lm].imag() * dI_imag);
            }
          }
        }
        // div by r
        ...
        // compute forces
        const FReal forceX =
            (cosPhi * sph.getSinTheta() * Fr + cosPhi * sph.getCosTheta() * FO +
             (-sinPhi) * Fp) *
            physicalValue;

        const FReal forceY = (sinPhi * sph.getSinTheta() * Fr +
                              sinPhi * sph.getCosTheta() * FO + cosPhi * Fp) *
                             physicalValue;

        const FReal forceZ =
            (sph.getCosTheta() * Fr + (-sph.getSinTheta()) * FO) *
            physicalValue;
      ...
      // compute the potential
      {
        ...
        for (int l = 0; l <= P; ++l) {
          ...
          for (int m = 1; m <= l; ++m, ++index_lm) {
            ...
            const FReal I_real = coef * cos_m_phi_i_pow_m[m];
            const FReal I_imag = coef * sin_m_phi_i_pow_m[m];
            magnitude += FReal(2.0) * (u[index_lm].real() * I_real -
                                       u[index_lm].imag() * I_imag);
          }
        }
        // inc potential
  ...
  void P2P(const FTreeCoordinate &inPosition,
    ...
    if (inTargets == inSources) {
      FP2PRTCutoff<FReal>::template Inner<ContainerClass>(inTargets);
      P2POuter(inPosition, inTargets, inNeighbors, neighborPositions, inSize);
    } else {
      const ContainerClass *const srcPtr[1] = {inSources};
      FP2PRTCutoff<FReal>::template FullRemote<ContainerClass>(inTargets,
                                                               srcPtr, 1);
      FP2PRTCutoff<FReal>::template FullRemote<ContainerClass>(
          inTargets, inNeighbors, inSize);
    }
  }
  ...
  void P2POuter(const FTreeCoordinate & /*inLeafPosition*/,
                ...
                const int neighborPositions[], const int inSize) override {
    ...
    FP2PRTCutoff<FReal>::template FullMutual<ContainerClass>(
        inTargets, inNeighbors, nbNeighborsToCompute);
  }
...
#endif // FROTATIONKERNEL_CUTOFF_HPP
