...
public:
  ...
  T getEnergy(const Vec3 &kCart, std::size_t band, std::int64_t &tetHint) const {
    ...
    const T e0 = E[tet[0]];
    return e0 + lam[0] * (E[tet[1]] - e0) + lam[1] * (E[tet[2]] - e0) +
           lam[2] * (E[tet[3]] - e0);
  }
  ...
  std::size_t getNrFallbackScans() const { return fallbackScans; }
  /// most negative barycentric coordinate ever accepted via shell-snapping
  T getWorstShellDeficit() const { return worstShellDeficit; }
...
private:
  ...
  mutable std::size_t fallbackScans = 0;
  mutable T worstShellDeficit = 0;
  ...
  std::int64_t locate(const Vec3 &p, std::int64_t &hint, Vec3 &lam) const {
    ...
    worstShellDeficit = std::min(worstShellDeficit, bestWorst);
    lam = bestLam;
    hint = bestTet;
    return bestTet;
  }
