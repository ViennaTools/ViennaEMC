...
int
oooWritePotential(const_t constpar, geometry_t *geometry, phys_quant_t *phys_quantities, int flagSolution, int iter) {
    static int i, j;
...
/********************************************************************/
output_t *
oooCalculateOutput(const_t constpar, geometry_t *geometry, phys_quant_t *phys_quantities, int nTimeStepsAv, int simtime,
                   int iterTotal, currents_t *current_cumulative, curr_from_charge_t *current_from_charge)
...
//	outputdata->fieldXY_x[i][j] = 0.0;
                ...
                if (j == geometry->nymax) outputdata->fieldXY_y[i][j] = 0.0;
                    ...
                    outputdata->fieldXY_y[i][j] =
                            -(phys_quantities->potential[i][j] - phys_quantities->potential[i][j + 1])
                            / geometry->meshSize * constpar.vt;
            ...
            outputdata->currentdensityX[i][j] =
                    phys_quantities->curSumX[i][j] / ((double) (nTimeStepsAv) * geometry->deviceWidth);
            outputdata->currentdensityY[i][j] =
                    phys_quantities->curSumY[i][j] / ((double) (nTimeStepsAv) * geometry->deviceWidth);
            outputdata->currentdensity[i][j] = sqrt(phys_quantities->curSumX[i][j] * phys_quantities->curSumX[i][j] +
                                                    phys_quantities->curSumY[i][j] * phys_quantities->curSumY[i][j]) /
                                               ((double) (nTimeStepsAv) * geometry->deviceWidth);
...
int oooWriteOutput(output_t *outputdata, geometry_t *geometry, phys_quant_t *phys_quantities) {
    ...
    for (i = 0; i <= outputdata->x_max; ++i) {
        ...
        fprintf(output6, "%e \t %e \t %e \t %e \t %e \n", outputdata->fieldX[i], outputdata->fieldY[i],
                outputdata->fieldX2[i], outputdata->fieldY2[i], outputdata->density[i]);
        fprintf(output8, "%f \t %f \t %f \n", outputdata->velocityX[i], outputdata->velocityY[i],
                outputdata->energy[i]);
    ...
    for (i = 0; i < outputdata->iterTotal; i++) {
        fprintf(output14, "%f \t %f \t %f \t %f \t %f\n", outputdata->current_from_charge[i].Time,
                outputdata->current_cumulative[i].Is_cumul, outputdata->current_cumulative[i].Id_cumul,
                outputdata->current_cumulative[i].Is_momentary, outputdata->current_cumulative[i].Id_momentary);
        fprintf(output13, "%f \t %f \t %f \t %f \t %f \n", outputdata->current_from_charge[i].Time,
                outputdata->current_from_charge[i].sourceCurr, outputdata->current_from_charge[i].drainCurr,
    ...
    printf("Current = %.8e, Resistivity = %f, Rho = %.8e\n",
           ...
           (phys_quantities->inputVd - phys_quantities->inputVs) /
           outputdata->current_from_charge[outputdata->iterTotal - 1].sourceCurr,
