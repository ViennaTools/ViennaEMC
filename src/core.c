...
//  oooDistributePotential(constants, geometry, scatpar, phys_quantities);
...
//  oooWritePotential(constants, geometry, phys_quantities, 1); 
...
//  printf ("\nphys_quantities->inputVoltage_Vd   = %.8f\n", phys_quantities->inputVoltage_Vd);
...
/*
  for (i = 0; i <= geometry->nxmax; ++i)  
      ...
      {
          phys_quantities->fxField[i][j] = (phys_quantities->inputVoltage_Vd-phys_quantities->inputVoltage_Vs)/geometry->deviceLength * constants.vt;
          phys_quantities->fyField[i][j] = 0.;
          
          phys_quantities->localfieldx[i][j] = (phys_quantities->inputVoltage_Vd-phys_quantities->inputVoltage_Vs)/geometry->deviceLength * constants.vt;
          phys_quantities->localfieldy[i][j] = 0.;
          if ((j == geometry->nymax) && (i == geometry->nxmax)) break;
      }
  printf("done\n");
*/

  for (j = 1; j <= iterTotal; ++j)
  {
    Time = j * scatpar->dt;
...
//    oooDistributePotential(constants, geometry, scatpar, phys_quantities);
    ...
    current_cumulative[j - 1] = oooVelocityEnergyCumulative(constants, geometry, scatpar, particles, phys_quantities, &Time, &nTimeStepsAv);  
...
//  oooWritePotential(constants, geometry, phys_quantities, 1);
  return oooCalculateOutput(constants, geometry, phys_quantities, nTimeStepsAv, (int)(tEnd - tBegin), iterTotal, current_cumulative, current_from_charge);
