...
/* (Dev.independent) 						    */
/********************************************************************/
int oooElectricFieldUpdate(const_t constpar, geometry_t *geometry, scatpar_t *scatpar, phys_quant_t *phys_quantities,
                           double *Time) {
    static int i, j;
    ...
    for (i = 0; i <= geometry->nxmax; ++i)
        for (j = 0; j <= geometry->nymax; ++j) {
            if (i == geometry->nxmax) {
                phys_quantities->fxField[i][j] =
                        (phys_quantities->potential[i - 1][j] - phys_quantities->potential[i][j])
                        / geometry->meshSize * constpar.vt;
                if (j == geometry->nymax)
                    phys_quantities->fyField[i][j] =
                            (phys_quantities->potential[i][j - 1] - phys_quantities->potential[i][j])
                            / geometry->meshSize * constpar.vt;
                else
                    phys_quantities->fyField[i][j] =
                ...
                phys_quantities->fxField[i][j] =
                        (phys_quantities->potential[i][j] - phys_quantities->potential[i + 1][j])
                        / geometry->meshSize * constpar.vt;
                phys_quantities->fyField[i][j] =
                        (phys_quantities->potential[i][j - 1] - phys_quantities->potential[i][j])
                        / geometry->meshSize * constpar.vt;
    ...
    for (i = 0; i <= geometry->nxmax; ++i)
        ...
        {
            if (i == geometry->nxmax)
                phys_quantities->localfieldx[i][j] =
                        (phys_quantities->fxField[i][j - 1] + phys_quantities->fxField[i][j]) * 0.5;
            else
                phys_quantities->localfieldx[i][j] =
                        (phys_quantities->fxField[i][j] + phys_quantities->fxField[i][j + 1]) * 0.5;

            if (j == geometry->nymax)
                phys_quantities->localfieldy[i][j] =
                        (phys_quantities->fyField[i - 1][j] + phys_quantities->fyField[i][j]) * 0.5;
            else
                phys_quantities->localfieldy[i][j] =
                        (phys_quantities->fyField[i][j] + phys_quantities->fyField[i + 1][j]) * 0.5;
        }
