...
/*  (Dev.independent) 						    */
/********************************************************************/
int oooInitializeDopingPotential(const_t constpar, geometry_t *geometry, scatpar_t *scatpar, phys_quant_t *phys_quantities)
{
...
//  printf("HERE!\n");
  for (i = 0; i <= geometry->nxmax; ++i)
    for (j = 0; j <= geometry->nymax; ++j)
    {
      ...
      phys_quantities->doping[i][j] = dopTerm;
      dopTerm *= 0.5;
...
//        printf("phys_quantities->inputVoltage_Vs = %.8f\n",phys_quantities->inputVoltage_Vs);
      phys_quantities->potential[i][j] = (phys_quantities->inputVoltage_Vs + i * (phys_quantities->inputVoltage_Vd - phys_quantities->inputVoltage_Vs) / 
              (geometry->nxmax + 1));// / constpar.vt;
    }
...
int oooPoissonSOR(const_t constants, geometry_t *geometry, phys_quant_t *phys_quantities, int flagSolution)
{
  ...
  do
  {
	...
	tempTerm = dennInv - denn + phys_quantities->doping[i][j] + phys_quantities->potential[i][j] * dennSum;
//	tempTerm = dennInv - denn + exp(constants.phi) + phys_quantities->potential[i][j] * dennSum;
//	tempTerm = dennInv - denn + phys_quantities->potential[i][j] * dennSum;
	...
	x1 = (forcingFunc[i][j] - termA - termB - termD - termE) / c2[i][j];	 					/* thesis eq. 4.11 */
	x2 = phys_quantities->potential[i][j] + constants.conv_omega * (x1 - phys_quantities->potential[i][j]);		/* thesis eq. 4.12 */
	newerror = fabs(x2 - phys_quantities->potential[i][j]);
	if (newerror > error)
	  error = newerror;
	phys_quantities->potential[i][j] = x2;
        //if (i==3) printf("phys_quantities->potential[%i][%i] = %f\n",i,j,phys_quantities->potential[i][j]);
